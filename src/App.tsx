        ...
        <div className="grid grid-cols-12 gap-2 mb-6 p-4 border border-dashed rounded-lg bg-slate-50 items-end">
          ...
          {newType === 'individual' && (
            <div className="col-span-6 md:col-span-2">
              ...
              <input type="number" min="1" max="10" className="w-full border p-2 rounded" value={newMax} onChange={e => setNewMax(parseInt(e.target.value) || 1)} />
            </div>
          )}
          <div className="col-span-12 md:col-span-3 flex items-end">
            <button onClick={handleAdd} className="w-full bg-blue-600 text-white py-2 rounded font-bold hover:bg-blue-700">➕ 新增</button>
          </div>
        </div>
          ...
          {localConfig.events.map((e: any) => (
            <div key={e.id} className="flex gap-2 items-center border p-2 rounded bg-white">
              ...
              <span className="font-bold flex-1">
                ...
                {e.type === 'individual' && <span className="ml-2 text-xs font-normal text-slate-500 border px-1 rounded">每班 {e.maxParticipants} 人</span>}
              </span>
              <button onClick={() => removeEvent(e.id)} className="text-red-500 px-2">🗑️</button>
            </div>
          ))}
        </div>
      </div>
