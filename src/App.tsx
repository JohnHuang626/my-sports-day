  ...
  return (
    <div className="space-y-6 pb-24 max-w-3xl mx-auto">
      ...
      <div className="bg-white p-6 rounded-xl shadow flex justify-between items-center">
        ...
        <button onClick={() => setLocalConfig((p:any) => ({...p, registrationOpen: !p.registrationOpen}))} className={`w-14 h-8 rounded-full transition-colors relative ${localConfig.registrationOpen !== false ? 'bg-green-500' : 'bg-slate-300'}`}>
           <div className={`absolute top-1 left-1 bg-white w-6 h-6 rounded-full transition-transform ${localConfig.registrationOpen !== false ? 'translate-x-6' : ''}`}></div>
        </button>
      </div>
