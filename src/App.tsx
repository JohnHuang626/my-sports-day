...
interface AppConfig {
  classes: ClassInfo[];
  events: SportEvent[];
  registrationOpen: boolean; // 報名總開關
}
...
export default function App() {
  ...
  const [isRecorderMode, setIsRecorderMode] = useState(false); // 紀錄員狀態
  ...
  useEffect(() => {
    ...
    const initAuth = async () => {
      try {
        ...
        console.warn("Auth blocked by browser (likely iframe/mobile restriction):", error);
        setIsOfflineMode(true);
      }
    };
    initAuth();
  ...
  useEffect(() => {
    ...
    const unsubConfig = onSnapshot(configRef, (docSnap) => {
      ...
      console.warn("Config permission denied:", error);
      setIsOfflineMode(true);
    });
    ...
    const unsubResults = onSnapshot(resultsRef, (docSnap) => {
      ...
      console.warn("Results permission denied:", error);
      setIsOfflineMode(true);
    });
    ...
    const unsubPasswords = onSnapshot(passwordsRef, (docSnap) => {
      ...
      console.warn("Passwords permission denied:", error);
      setIsOfflineMode(true);
    });
  ...
  const handleLoginSubmit = () => {
    ...
    if (loginTab === 'admin') {
      if (passwordInput === 'admin8888') { 
        ...
        setPasswordInput('');
      } else if (passwordInput === 'admin123') { // 紀錄員登入
  ...
  const isStaff = isAdminMode || isRecorderMode; // 大會工作人員
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 pb-20 print:bg-white print:pb-0">
      ...
      <main className="max-w-5xl mx-auto p-4 print:hidden">
        ...
        {currentView === 'class_registration' && loggedInClass && <ClassRegistration config={config} results={results} isOffline={isOfflineMode} setResults={setResults} classId={loggedInClass} passwords={passwords} setPasswords={setPasswords} />}
      </main>

      {/* 列印專用報表 (根據當前所在頁面切換印出的內容) */}
...
// --- 各班專屬報名表組件 ---
function ClassRegistration({ config, results, isOffline, setResults, classId, passwords, setPasswords }: any) {
  ...
  return (
    <div className="bg-white rounded-xl shadow p-6 max-w-3xl mx-auto mt-4">
      ...
      <div className="flex items-center gap-4">
        ...
        {msg && <span className="font-bold text-sm animate-fade-in text-slate-600">{msg}</span>}
      </div>
        ...
        <div className="space-y-2 max-h-96 overflow-y-auto">
          {localConfig.events.map((e: any) => (
            <div key={e.id} className="flex gap-2 items-center border p-2 rounded bg-white">
              ...
              <span className="font-bold flex-1">{getEventDisplayName(e)}</span>
...
function PrintRegistration({ config, results }: any) {
  ...
  return (
    <div className="hidden print:block p-8 bg-white text-black">
      <h1 className="text-3xl font-bold text-center mb-6">嘉新國中運動會 - 選手報名名單</h1>

      {[7, 8, 9].map(grade => {
        const gradeClasses = config.classes.filter((c: any) => c.grade === grade);
        return (
          <div key={grade} className="mb-10">
            <h2 className="text-2xl font-bold border-b-2 border-black pb-2 mb-4">{grade} 年級 個人賽名單</h2>
